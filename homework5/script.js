...
function symbolMap() {
    ...
    var color = d3.scale.threshold()
        .domain([10, 35, 84 ])
        .range(["#fecc5c","#fd8d3c","#e31a1c","#BD0026"]);
    ...
    function chart(id) {
        ...
        console.log(bbox);
         //var projection = d3.geo.equirectangular().scale(150).translate([bbox.width/2, bbox.height/2]).precision(.1);

         var projection = d3.geo.naturalEarth().scale(150).translate([bbox.width/2, bbox.height/2]).precision(.1);
        ...
        //
        //
        //country.call(zoom).call(zoom.event);
        ...
        svg.append('path').datum({type:"Sphere"}).attr("d", path).style("fill","none").style("stroke","#444444");
        ...
        //console.log(topojson.feature(map, map.objects.));
        ...
        symbols.selectAll("circle")
            ...
            .classed({"symbol": true}).style("fill", function(d){return color(+d.depth);})//.style("stroke","black")
    ...
    function showHighlight(d) {
        ...
        updateLog("At " + d.updated + ", " + d.place + " happened earthquake with magnitude " + d.mag + " and depth " + d.depth + " kilometers"
